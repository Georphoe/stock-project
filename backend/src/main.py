...
from fastapi.middleware.cors import CORSMiddleware # 匯入 CORS middleware（跨來源資源共享） # 用來解決「前端（不同 port / domain）無法呼叫後端 API」的問題

# 啟動 API
# 在你的**後端專案資料夾（有 main.py 的地方）**打開終端機（Terminal / Command Prompt / PowerShell），然後執行。
...
app.add_middleware(
    ...
    # ✅ 允許哪些來源（origin）可以呼叫你的 API
    # "*" 代表「全部允許」（開發方便，但不安全）
    # TODO: 上線時要改成你的前端網址，例如：
    # ["http://localhost:3000"] 或 ["https://your-frontend.com"]
    allow_origins=["*"],  

    # ✅ 允許哪些 HTTP 方法（method）
    # "*" = GET, POST, PUT, DELETE... 全部允許
    allow_methods=["*"],

    # ✅ 允許哪些 request headers
    # "*" = 允許所有 header（例如 Authorization, Content-Type）
    allow_headers=["*"],
)
